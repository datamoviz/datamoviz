...
module.exports = function (app, router) {
  ...
  router.get('/aggregate/years', (req, res) => {
    ...
    return app.get('mongoClient').then(db => {
      ...
      return Promise.all([startPromise, endPromise, aggregatePromise])
        .then(([start, end, aggregate]) => {
          ...
          years.unshift('years');
          return res.json([yearsList, years]);
        });
    });
  });

  router.get('/aggregate/years-colors', (req, res) => {
    const filters = parse(req.query.filters);
    ...
    return app.get('mongoClient').then(db => {
      ...
      return Promise.all([startPromise, endPromise, aggregatePromise])
        .then(([start, end, aggregate]) => {
          ...
          color.unshift('color');
          return res.json([blackAndWhite, color]);
        });
    });
  });

  router.get('/aggregate/genres', (req, res) => {
    const filters = parse(req.query.filters);
    ...
    return app.get('mongoClient').then(db => {
      ...
      return Promise.all([countPromise, genresPromise])
        .then(([total, genres]) => {
          ...
          return res.json(displayedGenres);
        })
    });
  });

  router.get('/aggregate/colors', (req, res) => {
    const filters = parse(req.query.filters);
    ...
    return app.get('mongoClient').then(db => {
      return db.collection('movies').aggregate([
        ...
        return res.json(formattedColors);
      });
    });
  });
};
